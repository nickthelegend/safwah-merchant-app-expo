...
export const TOKEN_META: Record<PayToken, { name: string; color: string; aed: number }> = {
  AED: { name: 'Dirham', color: '#15300C', aed: 1 },
...
export function tokenBreakdown(sales: Sale[]): { label: string; value: number; color: string }[] {
  return (['USDT', 'ETH', 'AED'] as PayToken[])
    .map((t) => ({ label: t, value: +sales.filter((s) => s.token === t).reduce((a, s) => a + s.amountAED, 0).toFixed(2), color: TOKEN_META[t].color === '#15300C' ? '#15300C' : TOKEN_META[t].color }))
    .filter((d) => d.value > 0);
}
